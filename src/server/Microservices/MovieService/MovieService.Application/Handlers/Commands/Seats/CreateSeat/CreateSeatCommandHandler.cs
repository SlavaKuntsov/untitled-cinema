	...
	IMapper mapper) : IRequestHandler<CreateSeatCommand, Guid>
{
	public async Task<Guid> Handle(CreateSeatCommand request, CancellationToken cancellationToken)
	{
		...
		var hallEntity = await unitOfWork.HallsRepository.GetAsync(request.Hall, cancellationToken)
						?? throw new NotFoundException($"Hall with name '{request.Hall}' not found.");

		var seatTypeEntity = await unitOfWork.SeatsRepository.GetTypeAsync(request.SeatType, cancellationToken)
		...
		await unitOfWork.Repository<SeatEntity>().CreateAsync(mapper.Map<SeatEntity>(seat), cancellationToken);
