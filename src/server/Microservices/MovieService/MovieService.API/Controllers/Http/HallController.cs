...
public class HallController(IMediator mediator) : ControllerBase
{
	...
	{
		var halls = await mediator.Send(new GetAllHallsQuery(), cancellationToken);
	...
	{
		var halls = await mediator.Send(new GetHallByIdQuery(id), cancellationToken)
	...
	public async Task<IActionResult> Create(
		[FromBody] CreateSimpleHallCommand requests,
		CancellationToken cancellationToken)
	{
		var movie = await mediator.Send(requests, cancellationToken);
	...
	public async Task<IActionResult> Create(
		[FromBody] CreateCustomHallCommand request,
		CancellationToken cancellationToken)
	{
	...
	public async Task<IActionResult> Update(
		[FromBody] UpdateHallCommand request,
		CancellationToken cancellationToken)
	{
		var movie = await mediator.Send(request, cancellationToken);
	...
	{
		await mediator.Send(new DeleteHallCommand(id), cancellationToken);
